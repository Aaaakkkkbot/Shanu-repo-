...
# Initialize backup system only when BACKUP_MONGO_URL is configured
backup_system = AutoBackup() if backup_ddw is not None else None
...
async def manual_backup(client: Client, message: Message):
    ...
    if backup_system is None:
        await message.reply_text("❌ Backup system is not configured. Set BACKUP_MONGO_URL to enable backups.")
        return
    try:
